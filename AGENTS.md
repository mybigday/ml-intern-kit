...
| Linux + AMD Strix Halo (gfx1151 / Ryzen AI Max+ 395) | `make env-strix-halo` → `train_sft.py` | gfx1151 is **not** in upstream PyTorch wheels. Bootstrap pulls from AMD TheRock nightly index (`rocm.nightlies.amd.com/v2/gfx1151/`). Up to ~96 GB unified LPDDR5X for GPU — full bf16 SFT of 7B is tractable. See `.claude/skills/rocm-strix-halo/SKILL.md`. |
